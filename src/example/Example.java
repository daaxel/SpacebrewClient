...
public class Example implements SpacebrewClientCallback
{
	...
	private Logger			log	= LoggerFactory.getLogger(SpacebrewClient.class);

	// The client instance we will sendand receive data from
	private SpacebrewClient	client;
	...
	public void onOpen()
	{
		...
		/*
		 * A small alpha value for the low-pass filter creates high smoothing. Because we smooth a
		 * random value, the output value will always try to approach the middle of
		 * the range interval [0,1023], which is 512.
		 */
	...
	private void loop()
	{
		...
		while (true)
		{
			...
			{
				client.send("switch", booleanValue);
				myBooleanPublisher.publish(booleanValue);
				booleanValue = !booleanValue;

				client.send("counter", rangeValue);
				client.send("counter", Integer.toBinaryString(rangeValue));
